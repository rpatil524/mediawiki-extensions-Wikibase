...
class ApiErrorReporterTest extends MediaWikiIntegrationTestCase {
	...
	protected function assertUsageException(
		...
		$data = $msg->getApiData();
		foreach ( $expectedData as $path => $value ) {
			$path = explode( '/', $path );
	...
	public function testDieException(
		...
		try {
			...
			$this->assertUsageException( $infoPattern, $code, $httpStatusCode, $expectedData, $ex );
		}
	}
	...
	public function testDieWithError(
		...
		try {
			...
			$this->assertUsageException( $infoPattern, $code, $httpRespCode, $expectedData, $ex );
		}
	}
	...
	public function testDieStatus(
		...
		try {
			...
			$this->assertUsageException( $infoPattern, $code, 0, $expectedData, $ex );
		}
	}
	 ...
	 */
	public function testReportStatusWarnings( Status $status, array $expectedData ) {
		$this->setUserLang( 'de' );
		$api = new ApiMain();
		$localizer = $this->getExceptionLocalizer();
		$reporter = new ApiErrorReporter( $api, $localizer );
